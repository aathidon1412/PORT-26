  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = startIdx; idx < config.count; idx++) {
      ...
      if (config.gravity === 0) {
        ...
        // Reset ball to top when it reaches bottom
        pos.y = config.maxY + radius;
        pos.x = MathUtils.randFloatSpread(2 * config.maxX * 0.8);
        vel.y = -Math.abs(vel.y) * 0.5;
        vel.x = MathUtils.randFloatSpread(0.1);
      }
...
class Y extends MeshPhysicalMaterial {
  ...
  constructor(params: any) {
    ...
    (this as MeshPhysicalMaterial).onBeforeCompile = (shader) => {
...
class Z extends InstancedMesh {
  ...
  #setupLights() {
    ...
    (this as InstancedMesh).add(this.ambientLight);
    this.light = new PointLight(this.config.colors[0], this.config.lightIntensity);
    (this as InstancedMesh).add(this.light);
  }
  ...
  setColors(colors: number[]) {
    if (Array.isArray(colors) && colors.length > 1) {
      ...
      for (let idx = 0; idx < mesh.count; idx++) {
        mesh.setColorAt(idx, colorUtils.getColorAt(idx / mesh.count));
        if (idx === 0) {
          this.light!.color.copy(colorUtils.getColorAt(idx / mesh.count));
        }
      }
